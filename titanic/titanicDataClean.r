...
## ----tplot, echo = FALSE-------------------------------------------------
titanic_train$FL <- stri_extract_first_regex(titanic_train$Ticket, "[A-Z]+") #capta primero la ocurrencia del texto
titanic_train$FT <- stri_extract_first_regex(titanic_train$Ticket, "[0-9][0-9]+") #toma el numero de ticket
...
## ----cabin, echo = FALSE-------------------------------------------------
sum(!substring(titanic_train$Cabin, 1, 1) == "")#Completa los datos de la cabina
datatable(data.frame(summary(titanic_train$Cabin)), colnames=c("Cabin", "Count")) # ¿Es seguro imputar los datos de la cabina?
...
#Además, hay un cambio del 72,4% de cualquier pasajero que embarca desde Southampton
...
kable(head(df), caption = "Dataset seleccionado",digits = 3, padding = 2, align = 'r')
datatable(df)
...
barplot(table(df$Embarked), xlab="Port of Embarkment", ylab="Frequencia", main="Histograma de Port of Embarkment")
...
plot(me_pclass, type="o", main="Efecto principal de Passenger Class", xlab="Passenger Class", ylab="Efecto principal",
     xaxt="n")
...
## ------------------------------------------------------------------------
interaction.plot(df$Pclass, df$Embarked, df$Survived, xlab="Passenger Class", ylab="Número promedio de sobrevivientes",
...
interaction.plot(df$Sex, df$Embarked, df$Survived, xlab="Sex", ylab="Número promedio de sobrevivientes", 
                 main="Efecto de interacción entre Sex y Port of Embarkment", legend=FALSE, xtick = FALSE, xaxt="n")
axis(1, c(1,2), labels=c("Female", "Male"))
legend("topright", c("Cherbourg","Queenstown","Southampton"), lty=c("dashed", "solid", "dotted"),
       title="Port of Embarkment")
...
interaction.plot(df$Age, df$Embarked, df$Survived, xlab="Age", ylab="Número promedio de sobrevivientes", 
                 main="Efecto de interacción entre Age y Port of Embarkment", legend=FALSE, xtick = FALSE, xaxt="n")
